...
public class ListClass {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < peopleList.size(); i++) {
			Person person = (Person) peopleList.get(i); // When using lists to store objects, the casting is needed to use the the object methods and attributes
			System.out.println(person.getInformation());
		}
		
		peopleList.remove(p1); // The method remove removes the object passed from the list
		System.out.println(peopleList.contains(p1)); // The method contains returns a boolean indicating if the passed object is in the list

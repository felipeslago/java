...
public class TreatingExceptions {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // In order words it says that may not be possible to get a field with this name, so the exception must the
            // treated
            ...
            // In order words it says that may not be possible to get a method with this name and parameters, so the
            // exception must the treated
            // If a method is set to private, this exception can be thrown

            Method method = c.getMethod("methodName", c);

            // When try invoking a method it throws IllegalArgumentException, IllegalAccessException and
            // InvocationTargetException
            // IllegalArgumentException: the passed parameters are not compatible with the method parameters (could be
            // the parameter type and/or the number of parameters)
            // IllegalAccessException: there is no permission to access the method (maybe the method is set to private)
            // InvocationTargetException: encapsulates the error when the exception was thrown by the method or
            // constructor invoked

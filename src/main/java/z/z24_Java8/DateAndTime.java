...
public class DateAndTime {
    ...
    public static void main(String[] args) {
        ...
        LocalDate date = LocalDate.of(2020, Month.JANUARY, 1); // The method of returns a new date based on the
                                                               // arguments passed: year, month and day
        System.out.println(date);

        int years = date.getYear() - today.getYear(); // The method getYear returns the year of the date and it is
                                                      // possible to calculate the difference between another date
        System.out.println(years);

        // But the Period class helps to calculate the difference between dates

        Period period = Period.between(today, date);
        System.out.println(period); // It returns a string like P3Y6M4D where P is Period, Y is year, M is month and D
                                    // is day, and the numbers in front indicates its values

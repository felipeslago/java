...
public class Streams {
    ...
    public static void main(String[] args) {
        ...
        s = companies.stream().filter(c -> c.getEmployees() > 100); // Using lambda to specify a criteria that gets all
        ...
        companies.stream().filter(c -> c.getEmployees() > 100).forEach(c -> System.out.println(c.getName()));
        ...
        companies.stream().filter(c -> c.getEmployees() > 100).map(c -> c.getEmployees()) // Creating a map just with
                                                                                          // the total of employees
                .forEach(e -> System.out.println(e));

        // And it is possible to simplify the code by using the Method References

        companies.stream().filter(c -> c.getEmployees() > 100).map(Company::getEmployees).forEach(System.out::println);
        ...
        // So the Stream interface offers a mapToInt method that returns a IntStream and offers specific int methods,
        // like doWork
        ...
        int sum = companies.stream().filter(c -> c.getEmployees() > 100).mapToInt(Company::getEmployees).sum();
        ...
        Optional<Company> o = companies.stream().filter(c -> c.getEmployees() > 100).findAny();
        ...
        companies = companies.stream().filter(c -> c.getEmployees() > 100).collect(Collectors.toList());

        // The toMap method generates a new Map like below
        // It receives two arguments to form the key value pair

        Map<String, Integer> m = companies
                .stream()
                .filter(c -> c.getEmployees() > 100)

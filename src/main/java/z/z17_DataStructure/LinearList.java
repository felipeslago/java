...
public class LinearList {
    ...
    void insertAt(int position, int data) { // This function inserts an element by a given position
        ...
        if (position < this.first || position > this.last) {
            ...
            for (int i = this.last; i >= position; i--) { // But inserting an element implies to move forward all
                                                          // elements from backward
                this.numbers[i] = this.numbers[i - 1]; // In other words, if an element is inserted at position 5, the
                                                       // element at this position will forward to position 6 and so on
            }
    ...
    void remove(int data) { // This function removes an element by its value
        for (int i = this.first; i < this.last; i++) { // The function must go through the list seeking the value
            if (this.numbers[i] == data) { // And when find it, removes the element
                for (int j = i; j < this.last; j++) { // But removing an element implies to move backwards all elements
                                                      // from forward
                    this.numbers[j] = this.numbers[j + 1]; // In other words, if the element at position 5 was removed,
                                                           // the element at position 6 will backward to position 5 and
                                                           // so on
                }
                this.last = this.last + 1;
                return;
